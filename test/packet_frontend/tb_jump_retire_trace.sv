...
module tb_jump_retire_trace;
    ...
    initial begin
        ...
        request[109]=8'h0c; request[110]=8'h0d; request[111]=8'hf9; request[112]=8'h49;
        for (i = 0; i < 18; i = i + 1) retire[i] = 0;
        retire[0]=8'ha1; retire[1]=1; retire[2]=8'h12; retire[4]=8'h08;
        retire[6]=1; retire[10]=8'hfd; retire[11]=8'h07; retire[12]=8'hec; retire[13]=8'h34;
        retire[14]=8'had; retire[15]=8'h75; retire[16]=8'h57; retire[17]=8'h42;
        @(posedge clk); @(negedge clk); rst_n = 1;
        for (i = 0; i < 113; i = i + 1) send_byte(request[i]);
        wait (tx_count == 36); @(posedge clk); #1;
        if (dut.staged_result_crc !== 32'h34ec07fd) $fatal(1, "result CRC mismatch got=%08x", dut.staged_result_crc);
