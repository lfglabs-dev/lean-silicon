...
if timeout 180 yosys -s formal/lsc1_packet_frontend_one_cycle_boundary.ys > "$out/one-cycle-boundary.log" 2>&1; then
  echo PASS > "$out/one-cycle-boundary.status"
else
  echo UNRESOLVED > "$out/one-cycle-boundary.status"
fi
