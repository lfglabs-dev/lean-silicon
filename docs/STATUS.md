...
| Lean v1 packet/transaction foundation | protocol-sized request/response round trips, request validation precedence, atomic staging, abort/reset, matching retirement and exactly-once behavior proved for a pure functional model | `lean/LeanVMBMinCore/{Packet,Transaction}.lean`; byte-stream/per-opcode decoding, CRC-32 instantiation, service states, and RTL/oracle refinement remain open |
