...
| DEREF(alpha,beta,gamma,Cell) | Read pointer `P=L(alpha)`, require raw reverse lookup `P=encode(base)`, then `a2=base+beta`, `a3=L(gamma)`. If both cells are written they must agree; if one is written fill the other; if neither is written, defer equality. It has three accesses and `pc+1`. |
| DEREF(...,Pc) | At `a2`, write `encode(pc+2)`, not the misleading `pc+1` successor comment found in some summaries; three accesses and `pc+1`. |
| DEREF(...,Fp) | At `a2`, write `encode(fp)`; three accesses and `pc+1`. |
| JUMP(oc,od,of) | Read all of `c=L(oc), d=L(od), f=L(of)` and make all three access events. If `c=0`, next `(pc,fp)=(pc+1,fp)`; otherwise `d` and `f` MUST each reverse-resolve to valid raw `g` powers and next state is their indexes. The witness fields are `b=[c!=0]`, `w=c^-1` for nonzero `c`, otherwise `b=w=0`. |
