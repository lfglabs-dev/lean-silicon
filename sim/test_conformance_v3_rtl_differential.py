...
be mistaken for a lifecycle retire attempt.  A separate four-frame differential
removes the final payload byte from that RETIRE, declares length 7, and recomputes
the CRC.  It reaches the authored RTL's RETIRE length guard, preserves the pending
result after ``BAD_LENGTH``, and then accepts the untouched matching RETIRE.  A
companion four-frame differential appends exactly ``0xa5`` to the untouched
...
only outer envelope version byte 1
from ``0x01`` to ``0x02`` and recomputes the CRC.  It observes ``BAD_VERSION``
without losing or committing the pending result, then observes ``RETIRED`` for
the untouched nominal RETIRE.  The duplicate response is not the adapter-level
...
class ConformanceV3RtlDifferentialTests(unittest.TestCase):
    ...
    def test_short_retire_preserves_result_then_retry_retires(self) -> None:
        """A CRC-valid seven-byte RETIRE must stop at the length guard."""
        wire = self.nominal["wire"]
        request = bytes.fromhex(wire["blake3_request_hex"])
        service = bytes.fromhex(wire["service_response_frame_hex"])
        good_retire = bytes.fromhex(wire["retire_request_hex"])
        good_frame = request_from_wire(good_retire)
        short_retire = protocol.RequestFrame(
            protocol.Opcode.RETIRE, good_frame.payload[:-1]).encode()
        requests = [request, service, short_retire, good_retire]

        # This is a complete, CRC-valid request envelope.  It differs
        # semantically only by removing the final payload byte and changing the
        # declared payload length from 8 to 7; encode() supplies the fresh CRC.
        short_frame = request_from_wire(short_retire)
        self.assertEqual(len(good_frame.payload), 8)
        self.assertEqual(len(short_frame.payload), 7)
        self.assertEqual(short_frame.payload, good_frame.payload[:-1])
        self.assertEqual(short_retire[:4], good_retire[:4])
        self.assertEqual(short_retire[4:6], (7).to_bytes(2, "little"))
        self.assertEqual(
            int.from_bytes(short_retire[-4:], "little"),
            protocol.crc32(short_retire[:-4]),
        )
        self.assertNotEqual(short_retire[-4:], good_retire[-4:])
        ...
        self.assertEqual(
            [protocol.decode_response(raw).status for raw in model_responses],
            [protocol.Status.SERVICE_REQUIRED, protocol.Status.OK,
             protocol.Status.BAD_LENGTH, protocol.Status.RETIRED],
        )
        self.assertEqual((endpoint.committed_pc, endpoint.committed_fp,
                          endpoint.retire_seq, endpoint.state_valid),
                         (3, 0x40, 1, True))
        ...
        manifest = Path(self.temporary.name) / "short-retire-retry.manifest"
        manifest_lines = []
        for index, raw in enumerate(requests):
            path = Path(self.temporary.name) / f"short-retire-retry-{index}.hex"
            path.write_text("\n".join(f"{byte:02x}" for byte in raw) + "\n")
            manifest_lines.append(f"{path} {len(raw)}")
        manifest.write_text("\n".join(manifest_lines) + "\n")
        run = subprocess.run(
            ["vvp", str(self.simulator), f"+MANIFEST={manifest}",
             "+V3_FINITE_STALLS", "+V3_SHORT_RETIRE"],
            cwd=ROOT, check=True, capture_output=True, text=True)
        rtl_responses = [
            bytes.fromhex(line.removeprefix("RESPONSE "))
            for line in run.stdout.splitlines() if line.startswith("RESPONSE ")]
        ...
        self.assertEqual([line.split("done=")[1] for line in transactions],
                         ["0", "0", "0", "1"])
        pending = next(line for line in run.stdout.splitlines()
                       if line.startswith("RTL_V3_SHORT_RETIRE "))
        self.assertEqual(
            pending,
            "RTL_V3_SHORT_RETIRE payload_length=7 result_pending=1 "
    ...
    def test_oversized_retire_preserves_result_then_retry_retires(self) -> None:
        """A CRC-valid nine-byte RETIRE must stop at the length guard."""
        wire = self.nominal["wire"]
        request = bytes.fromhex(wire["blake3_request_hex"])
        service = bytes.fromhex(wire["service_response_frame_hex"])
        good_retire = bytes.fromhex(wire["retire_request_hex"])
        good_frame = request_from_wire(good_retire)
        oversized_retire = protocol.RequestFrame(
            protocol.Opcode.RETIRE, good_frame.payload + b"\xa5").encode()
        requests = [request, service, oversized_retire, good_retire]

        # The original eight payload bytes are untouched.  Only one trailing
        # 0xa5 byte is appended, length becomes 9, and encode() supplies a fresh
        # envelope CRC so the semantic RETIRE length guard is exercised.
        oversized_frame = request_from_wire(oversized_retire)
        self.assertEqual(len(good_frame.payload), 8)
        self.assertEqual(len(oversized_frame.payload), 9)
        self.assertEqual(oversized_frame.payload[:8], good_frame.payload)
        self.assertEqual(oversized_frame.payload[8:], b"\xa5")
        self.assertEqual(oversized_retire[:4], good_retire[:4])
        self.assertEqual(oversized_retire[4:6], (9).to_bytes(2, "little"))
        self.assertEqual(
            int.from_bytes(oversized_retire[-4:], "little"),
            protocol.crc32(oversized_retire[:-4]),
        )
        self.assertNotEqual(oversized_retire[-4:], good_retire[-4:])
        ...
        manifest = Path(self.temporary.name) / "oversized-retire-retry.manifest"
        manifest_lines = []
        for index, raw in enumerate(requests):
            path = Path(self.temporary.name) / f"oversized-retire-retry-{index}.hex"
            path.write_text("\n".join(f"{byte:02x}" for byte in raw) + "\n")
            manifest_lines.append(f"{path} {len(raw)}")
        manifest.write_text("\n".join(manifest_lines) + "\n")
        run = subprocess.run(
            ["vvp", str(self.simulator), f"+MANIFEST={manifest}",
             "+V3_FINITE_STALLS", "+V3_OVERSIZED_RETIRE"],
            cwd=ROOT, check=True, capture_output=True, text=True)
        rtl_responses = [
            bytes.fromhex(line.removeprefix("RESPONSE "))
            for line in run.stdout.splitlines() if line.startswith("RESPONSE ")]
        ...
        self.assertEqual([line.split("done=")[1] for line in transactions],
                         ["0", "0", "0", "1"])
        pending = next(line for line in run.stdout.splitlines()
                       if line.startswith("RTL_V3_OVERSIZED_RETIRE "))
        self.assertEqual(
            pending,
            "RTL_V3_OVERSIZED_RETIRE payload_length=9 result_pending=1 "
            "txn_id=10203040 result_crc=9d78969c next_pc=00000003 "
            "next_fp=00000040 state_valid=0 pc=00000000 fp=00000000 "
            "retire_seq=00000000 done=0 parser_state=0",
        )
    ...
    def test_retire_bad_flags_preserves_result_then_retry_retires(self) -> None:
        """Reject CRC-valid envelope flags 0x01, then accept untouched RETIRE."""
        wire = self.nominal["wire"]
        request = bytes.fromhex(wire["blake3_request_hex"])
        service = bytes.fromhex(wire["service_response_frame_hex"])
        good_retire = bytes.fromhex(wire["retire_request_hex"])
        good_frame = request_from_wire(good_retire)
        bad_retire = protocol.RequestFrame(
            good_frame.opcode, good_frame.payload, flags=0x01,
            version=good_frame.version).encode()
        requests = [request, service, bad_retire, good_retire]

        # Pin the semantic mutation to only outer envelope flags byte 3.
        # The fresh trailing CRC makes this a valid complete envelope that
        # reaches the receiver's flags admission check.
        self.assertEqual(len(bad_retire), len(good_retire))
        self.assertEqual(
            [index for index, (good, bad) in
             enumerate(zip(good_retire[:-4], bad_retire[:-4])) if good != bad],
            [3],
        )
        self.assertEqual((good_retire[3], bad_retire[3]), (0x00, 0x01))
        self.assertEqual(good_retire[1:3], bad_retire[1:3])
        self.assertEqual(good_retire[4:-4], bad_retire[4:-4])
        self.assertEqual(good_retire[2], protocol.Opcode.RETIRE)
        self.assertEqual(good_retire[3], 0x00)
        self.assertEqual(int.from_bytes(good_retire[4:6], "little"), 8)
        self.assertEqual(
            int.from_bytes(bad_retire[-4:], "little"),
            protocol.crc32(bad_retire[:-4]),
        )
        self.assertNotEqual(bad_retire[-4:], good_retire[-4:])
        bad_frame = request_from_wire(bad_retire)
        self.assertEqual(bad_frame.version, good_frame.version)
        self.assertEqual(bad_frame.opcode, good_frame.opcode)
        self.assertEqual(bad_frame.flags, 0x01)
        self.assertEqual(bad_frame.payload, good_frame.payload)
        ...
        self.assertEqual(
            [protocol.decode_response(raw).status for raw in model_responses],
            [protocol.Status.SERVICE_REQUIRED, protocol.Status.OK,
             protocol.Status.BAD_FLAGS, protocol.Status.RETIRED],
        )
        self.assertEqual((endpoint.committed_pc, endpoint.committed_fp,
                          endpoint.retire_seq, endpoint.state_valid),
                         (3, 0x40, 1, True))
        ...
        manifest = Path(self.temporary.name) / "bad-flags-retire-retry.manifest"
        manifest_lines = []
        for index, raw in enumerate(requests):
            path = Path(self.temporary.name) / f"bad-flags-retire-retry-{index}.hex"
            path.write_text("\n".join(f"{byte:02x}" for byte in raw) + "\n")
            manifest_lines.append(f"{path} {len(raw)}")
        manifest.write_text("\n".join(manifest_lines) + "\n")
        run = subprocess.run(
            ["vvp", str(self.simulator), f"+MANIFEST={manifest}",
             "+V3_FINITE_STALLS", "+V3_BAD_FLAGS_RETIRE"], cwd=ROOT,
        ...
        self.assertEqual([line.split("done=")[1] for line in transactions],
                         ["0", "0", "0", "1"])
        pending = next(line for line in run.stdout.splitlines()
                       if line.startswith("RTL_V3_BAD_FLAGS_RETIRE "))
        self.assertEqual(
            pending,
            "RTL_V3_BAD_FLAGS_RETIRE result_pending=1 txn_id=10203040 "
            "result_crc=9d78969c next_pc=00000003 next_fp=00000040 "
            "state_valid=0 pc=00000000 fp=00000000 retire_seq=00000000 "
            "done=0 parser_state=0",
        )
    ...
    def test_retire_bad_version_preserves_result_then_retry_retires(self) -> None:
        """Reject CRC-valid version 0x02, then accept the untouched RETIRE."""
        wire = self.nominal["wire"]
        request = bytes.fromhex(wire["blake3_request_hex"])
        service = bytes.fromhex(wire["service_response_frame_hex"])
        good_retire = bytes.fromhex(wire["retire_request_hex"])
        ...
        # Pin the semantic mutation to only outer protocol version byte 1.
        # The fresh trailing CRC makes this a valid complete envelope that
        # reaches the receiver's version admission check.
        self.assertEqual(len(bad_retire), len(good_retire))
        self.assertEqual(
            [index for index, (good, bad) in
             enumerate(zip(good_retire[:-4], bad_retire[:-4])) if good != bad],
        ...
        self.assertEqual((good_retire[1], bad_retire[1]), (0x01, 0x02))
        self.assertEqual(good_retire[2:-4], bad_retire[2:-4])
        self.assertEqual(good_retire[2], protocol.Opcode.RETIRE)
        self.assertEqual(good_retire[3], 0x00)
        self.assertEqual(int.from_bytes(good_retire[4:6], "little"), 8)
        self.assertEqual(
        ...
        self.assertNotEqual(bad_retire[-4:], good_retire[-4:])
        bad_frame = request_from_wire(bad_retire)
        self.assertEqual(bad_frame.version, 0x02)
        self.assertEqual(bad_frame.opcode, good_frame.opcode)
        self.assertEqual(bad_frame.flags, good_frame.flags)
        self.assertEqual(bad_frame.payload, good_frame.payload)
        ...
        self.assertEqual(
            [protocol.decode_response(raw).status for raw in model_responses],
            [protocol.Status.SERVICE_REQUIRED, protocol.Status.OK,
             protocol.Status.BAD_VERSION, protocol.Status.RETIRED],
        )
        self.assertEqual((endpoint.committed_pc, endpoint.committed_fp,
                          endpoint.retire_seq, endpoint.state_valid),
                         (3, 0x40, 1, True))
        ...
        manifest = Path(self.temporary.name) / "bad-version-retire-retry.manifest"
        manifest_lines = []
        for index, raw in enumerate(requests):
            path = Path(self.temporary.name) / f"bad-version-retire-retry-{index}.hex"
            path.write_text("\n".join(f"{byte:02x}" for byte in raw) + "\n")
            manifest_lines.append(f"{path} {len(raw)}")
        manifest.write_text("\n".join(manifest_lines) + "\n")
        run = subprocess.run(
            ["vvp", str(self.simulator), f"+MANIFEST={manifest}",
             "+V3_FINITE_STALLS", "+V3_BAD_VERSION_RETIRE"], cwd=ROOT,
            check=True, capture_output=True, text=True)
        rtl_responses = [
            bytes.fromhex(line.removeprefix("RESPONSE "))
            for line in run.stdout.splitlines() if line.startswith("RESPONSE ")]
        ...
        self.assertEqual([line.split("done=")[1] for line in transactions],
                         ["0", "0", "0", "1"])
        pending = next(line for line in run.stdout.splitlines()
                       if line.startswith("RTL_V3_BAD_VERSION_RETIRE "))
        self.assertEqual(
            pending,
            "RTL_V3_BAD_VERSION_RETIRE result_pending=1 txn_id=10203040 "
            "result_crc=9d78969c next_pc=00000003 next_fp=00000040 "
            "state_valid=0 pc=00000000 fp=00000000 retire_seq=00000000 "
            "done=0 parser_state=0",
        )
